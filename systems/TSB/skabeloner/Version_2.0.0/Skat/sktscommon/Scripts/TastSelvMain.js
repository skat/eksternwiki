    ...
    $.showSpecific = function (el) {
      ...
      $.each(el.closest('.skts-layout2-input-gruppe').find('input'), function() {
        ...
        if(!$(this).prop("checked")) {
          $(fold).slideUp().addClass('skts-visualhidden');
          fold.switchKbdNavigable(false);
        }
     });
   ...
   $.foldIt = function (el) { 
      ...
      if(type === "radio") {
        $.each(el.closest('.skts-layout2-input-gruppe').find(':radio'), function() {
          if(!$(this).prop("checked"))
            $(this).nextUntil('.skts-fold').next().addClass('skts-visualhidden').slideUp();
       });
      }
      ...
      */
      function setClick(el) {
        ...
        if(details.hasClass('skts-visualhidden')) {
          ...
          details.removeClass('skts-visualhidden').hide().slideDown(500, function() { details.switchKbdNavigable(true); });
        } else {
          details.slideUp(500, function() { $(this).addClass('skts-visualhidden'); details.switchKbdNavigable(false); });
        }
        arrow.focus();
        return false;
      }
