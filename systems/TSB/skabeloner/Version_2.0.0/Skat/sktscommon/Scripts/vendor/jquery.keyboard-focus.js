...
*
* 20130627: Modified by Gregers Hovmand to handle mouseover events aswell
*/
    ...
    */
    $.fn.trackFocus = function () {
      ...
      .bind('focusout', function(e){
        $('.' + $.keyFocus.conf.keyFocusClass + ', .' 
        + $.keyFocus.conf.mouseFocusClass).removeClass($.keyFocus.conf.keyFocusClass + ' ' 
        + $.keyFocus.conf.mouseFocusClass);
        $(e.target).removeClass($.keyFocus.conf.focusClass);
      })
      .bind('mouseover', function(e) {
        if($(e.target).is('a')) {
          if($(e.target).hasClass('skts-tooltip-holder')) {
            $('a').each(function() {
              $(this).removeClass($.keyFocus.conf.keyFocusClass);
            });
          }
        }
      });
