    ...
    $.showSpecific = function (el) {
      ...
      $.each(el.closest('.skts-layout2-input-gruppe').find('input'), function() {
        ...
        if(!$(this).prop("checked")) {
          $(fold).addClass('skts-visualhidden').switchKbdNavigable(false);
        }
      });
      foldDo(el, fold);
    };
    ...
    $.foldIt = function (el) { 
      ...
      if(type === "radio") {
        $.each(el.closest('.skts-layout2-input-gruppe').find(':radio'), function() {
          if(!$(this).prop("checked"))
            $(this).nextUntil('.skts-fold').next().addClass('skts-visualhidden');
            $(this).nextUntil('.skts-fold').next().find("input, select, a").attr("tabindex", "-1");
        });
      }
...
function openCookieDialog(url, winName) {
    var url = url || 'http://www.skat.dk/SKAT.aspx?oId=1617453&layout=353121';
    var winName = winName || 'Om Cookies';
...
/*
*
* Sekundære vinduer: popup og hjæl-popup
*
*/
/*****************************************************************************************************************/
var hjaelp = null;
...
if ((screen.width == 640) && (screen.height == 480)) {
  ...
  var w1 = 650; var h1 = 355; var l1 = 20; var t1 = 20;
}
...
function aabn_hjaelp_vindue(file) {
  ...
  hjaelp = window.open(file,'Hjaelp',krom);
  hjaelp.focus();
}

function popupvindue(url) 
{
 newwin=window.open(url,'_blank', krom);
